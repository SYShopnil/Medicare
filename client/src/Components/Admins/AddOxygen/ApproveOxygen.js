...
export default function ApproveOxygen() {
  ...
  const approveHandler = async (e, data) => {
    ...
    const id = data._id
    const approveReq = await axios.post(`${baseUrl}/oxygenCylinder/request/approve/${id}`, body, header)
  ...
  return (
    <div>
      ...
      {unApprovalData.length === 0 ? (
        <h2 className = {`p-3 bg-danger text-center text-light`}>No new request</h2>
      ) : (
        <>
